...
test('every setting the lock screen reads can actually be set', async () => {
  ...
  const KEYS = ['DEVICE_LOCK_BRAND', 'DEVICE_LOCK_MESSAGE', 'DEVICE_HELP_PHONE',
    'DEVICE_OFFLINE_GRACE_HOURS', 'DEVICE_FRP_ACCOUNT_IDS'];
...
/* =========================================================================================
   TRANSFERS -- stock changing hands inside HOOP (2026-09-16 .. 18):
     ...
     "1. Sipho / Store transfers them to RSM  2. RSM to Agents  3. Agent to RSM (the returns)
      4. RSM / Agent to Sipho / Store  5. RSM to RSM"
     ...
     "sender must be current account settings. role store = superagent. ... always pull them
      auto from new stock whenever they appear and auto insert them in access codes with
      autogenerated code ... price should pull from new stock if there unless dont fill"
   Schema: db/migrations/RUN-ME-2026-09-16-transfers.sql + RUN-ME-2026-09-17-transfers-flow.sql;
   the one-off staff/codes backfill is RUN-ME-2026-09-18-staff-from-stock.sql.
   ========================================================================================= */
...
function trDb(extra, opts) {
  return fakeDb(Object.assign({
    ...
    hoop_agents: [
      { name: 'RSM DAR', role: 'Regional_Manager', branch: 'Dar es salaam', phone: '0700000001' },
      { name: 'RSM MWANZA', role: 'Regional_Manager', branch: 'Mwanza', phone: '0700000002' },
      { name: 'SUPER AGENT', role: 'Regional_Manager', branch: null, phone: '0486516049' },
      { name: 'AGENT ONE', role: 'Field_Officer', branch: 'Dar es salaam', phone: '0700000011' },
      { name: 'AGENT TWO', role: 'Field_Officer', branch: 'Dar es salaam', phone: '0700000012' },
      { name: 'AGENT THREE', role: 'Field_Officer', branch: 'Mwanza', phone: '0700000013' },
      { name: 'AGENT FOUR', role: 'Field_Officer', branch: 'Dar es salaam', manager: 'RSM MWANZA', phone: '0700000014' },
    ],
    devices: [
      { imei: '351000000000001', item: 'SAMSUNG A07-64GB', holder: 'SUPER AGENT', state: 'enrolled' },
      { imei: '351000000000002', item: 'SAMSUNG A07-64GB', holder: 'SUPER AGENT', state: 'enrolled' },
    ...
    // What NEW STOCK stamped: a price for two of the handsets, none for the rest.
    stock_audit: [
      { imei: '351000000000003', price: 450000, model: 'RIMO-64GB', rsm: 'RSM DAR', agent: 'RSM DAR' },
      { imei: '351000000000004', price: 380000, model: 'RIMO-64GB', rsm: 'RSM DAR', agent: 'AGENT ONE' },
    ],
...
test('the Send form\'s vocabulary: users grouped by role, every model the stock names, and the sender is you', async () => {
  const d = trDb();
  const r = await _FNS.transferUsers(d, AG1, {});
  assert.deepEqual(r.sender, { name: 'AGENT ONE', role: 'AGENT' }, '"sender must be current account settings"');
  assert.deepEqual(r.users.map(u => u.role + ':' + u.name), ['ADMIN:Peter', 'AGENT:AGENT FOUR', 'AGENT:AGENT THREE',
    'AGENT:AGENT TWO', 'RSM:RSM DAR', 'RSM:RSM MWANZA', 'STORE:SIPHO K'], 'by role then name, never yourself, never HR');
  assert.ok(r.users.every(u => !('code' in u)), 'a code is a secret; a name is what you send to');
  assert.deepEqual(r.models, ['ITEL A100', 'RIMO-64GB', 'SAMSUNG A07-64GB'], '"always prelist all existing models" -- register, old list, audit');
  assert.equal((await _FNS.transferUsers(d, STORE, {})).desk, true);
});

test('Send: the sender is the login, the receiver must be a system user, you send only what you hold', async () => {
  const d = trDb();
  ...
  assert.equal(line.source, 'devices'); assert.equal(line.prev_holder, 'AGENT ONE'); assert.equal(line.price, 100000);
  // The desk sends its own: the warehouse's stock, written under SUPER AGENT, and a serial nobody knows.
  const u = await _FNS.transferCreate(d, STORE, { toName: 'RSM DAR', imeis: ['351000000000001', '999999999999999'] });
  assert.equal(u.unknown, 1);
  const doc = d._dump('transfers').find(t => t.id === u.id);
  assert.equal(doc.from_name, 'SIPHO K'); assert.equal(doc.from_role, 'STORE');
  assert.equal(d._dump('transfer_items').find(l => l.imei === '999999999999999').source, 'unknown');
});

test('the price comes off NEW STOCK per handset when the box is blank, and the box wins when it is not', async () => {
  const d = trDb();
  // RSM DAR holds ...003 (stamped 450,000); the box is blank -> the stamp; a second handset with no
  // stamp goes on at zero rather than inventing one.
  const r = await _FNS.transferCreate(d, RSM_DAR, { toName: 'AGENT TWO', imeis: ['351000000000003'] });
  assert.equal(r.priced, 1);
  const lines = d._dump('transfer_items').filter(l => l.transfer_id === r.id);
  assert.equal(lines[0].price, 450000);
  assert.equal(d._dump('transfers').find(t => t.id === r.id).total_amount, 450000);
  const desk = await _FNS.transferCreate(d, STORE, { toName: 'RSM MWANZA', imeis: ['351000000000004', '351000000000001'] });
  assert.equal(desk.priced, 1, 'one had a stamp, one had not');
  const byImei = Object.fromEntries(d._dump('transfer_items').filter(l => l.transfer_id === desk.id).map(l => [l.imei, l.price]));
  assert.deepEqual(byImei, { '351000000000004': 380000, '351000000000001': 0 });
...
test('Receive: accepting signs, moves the holder on the register and the old list, and marks the document', async () => {
  ...
  await assert.rejects(() => _FNS.transferAccept(d, STORE, { id: s.id, signature: TR_PNG }), /not sent to you/,
    'not even the desk accepts for somebody: "they log in to sign"');
  await assert.rejects(() => _FNS.transferAccept(d, RSM_DAR, { id: s.id, signature: 'not-a-png' }), /not recognised/);
  assert.equal(rowOf(d, 'devices', '351000000000001').holder, 'SUPER AGENT', 'nothing moves before the signature');

  const a = await _FNS.transferAccept(d, RSM_DAR, { id: s.id, signature: TR_PNG });
  assert.deepEqual({ moved: a.moved, devices: a.devices, oldStock: a.oldStock, unknown: a.unknown, holder: a.holder },
    { moved: 2, devices: 1, oldStock: 1, unknown: 0, holder: 'RSM DAR' });
...
test('"role store = superagent": stock accepted by a STORE code is held by SUPER AGENT, the register\'s one name for the warehouse', async () => {
  ...
  const up = await _FNS.transferCreate(d, AG3, { toName: 'SIPHO K', imeis: ['351000000000005', '861000000000002'] });
  const a = await _FNS.transferAccept(d, STORE, { id: up.id, signature: TR_PNG });
  assert.equal(a.holder, 'SUPER AGENT');
  assert.equal(rowOf(d, 'devices', '351000000000005').holder, 'SUPER AGENT', 'not the clerk\'s own name');
  assert.equal(rowOf(d, 'old_stock', '861000000000002').agent, 'SUPER AGENT');
  assert.equal(rowOf(d, 'old_stock', '861000000000002').rsm, null, 'back at the desk it answers to nobody in the field');
  assert.equal(d._dump('transfers').find(t => t.id === up.id).receiver_signed_by, 'SIPHO K', 'the document still names who signed');
  // And the desk's Stock window shows it as the warehouse's, so it can go straight out again.
  assert.ok(imeisOf(await _FNS.transferStock(d, STORE, {})).includes('351000000000005'));
});
...
test('the sender signs at Send or later from their own login; the receiver only ever signs by accepting; nobody signs for another', async () => {
  ...
  const v = await _FNS.transferCreate(d, RSM_DAR, { toName: 'RSM MWANZA', imeis: ['351000000000003'] });
  await assert.rejects(() => _FNS.transferSign(d, AG1, { id: v.id, role: 'sender', signature: TR_PNG }), /not yours to sign/);
  await assert.rejects(() => _FNS.transferSign(d, STORE, { id: v.id, role: 'sender', signedBy: 'RSM DAR', signature: TR_PNG }),
    /not yours to sign/, 'the desk cannot sign in the sender\'s name either');
...
/* =========================================================================================
   REVERSAL -- "Some signatories on transfers are not okay so sipho needs a reversal button
   per user signature so that it gets resigned on every transfer card." Desk-only; clears one
   party's own signature and reopens the document; stock already moved is sent back the same
   way it came. The desk's OWN signature is never reversible (TR_SIG_ROLES) -- there is no
   door for it to be re-supplied on an existing document.
   ========================================================================================= */
test('reversing a signature is the desk\'s job only -- not the signer, not any other party', async () => {
  const d = trDb();
  const s = await _FNS.transferCreate(d, AG1, { toName: 'RSM DAR', imeis: ['351000000000004'], signature: TR_PNG });
  await assert.rejects(() => _FNS.transferReverseSignature(d, AG1, { id: s.id, role: 'sender', reason: 'wrong tap' }),
    /store desk.?s job/i, 'not even the party who signed it may reverse their own');
  await assert.rejects(() => _FNS.transferReverseSignature(d, RSM_DAR, { id: s.id, role: 'sender', reason: 'wrong tap' }),
    /store desk.?s job/i, 'not the other party either');
  const r = await _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'sender', reason: 'wrong tap' });
  assert.equal(r.ok, true);

  // ADMIN is the store desk everywhere else in this file (isStoreDesk) -- confirmed for real
  // here too, on a fresh document, rather than assumed.
  const s2 = await _FNS.transferCreate(d, AG1, { toName: 'RSM DAR', imeis: ['351000000000004'], signature: TR_PNG });
  const ADMIN = { code: 'X', name: 'Peter', role: 'ADMIN', teams: null, tabs: TR_TABS, readOnly: false };
  const r2 = await _FNS.transferReverseSignature(d, ADMIN, { id: s2.id, role: 'sender', reason: 'confirming ADMIN is the desk too' });
  assert.equal(r2.ok, true); assert.equal(r2.reversedBy, 'Peter');
});

test('the desk\'s own signature on a three-way document cannot be reversed -- there is no door to re-supply it', async () => {
  const d = trDb();
  const r = await _FNS.transferCreate(d, STORE, { toName: 'RSM MWANZA', fromName: 'RSM DAR', imeis: ['351000000000003'], signature: TR_PNG });
  await assert.rejects(() => _FNS.transferReverseSignature(d, STORE, { id: r.id, role: 'desk', reason: 'wrong desk clerk filed it' }),
    /desk.?s own signature cannot be reversed/i);
  assert.equal(d._dump('transfers').find(t => t.id === r.id).desk_signed_by, 'SIPHO K', 'untouched');
});

test('nothing to reverse is refused, and a reason is required', async () => {
  const d = trDb();
  const s = await _FNS.transferCreate(d, AG1, { toName: 'RSM DAR', imeis: ['351000000000004'] });   // unsigned
  await assert.rejects(() => _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'sender', reason: 'x' }),
    /has not been set yet/i);
  await _FNS.transferSign(d, AG1, { id: s.id, role: 'sender', signature: TR_PNG });
  await assert.rejects(() => _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'sender', reason: '   ' }),
    /reason is required/i);
});

test('reversing a signature on a document still SENT clears the one column and nothing else -- nothing had moved', async () => {
  const d = trDb();
  const s = await _FNS.transferCreate(d, AG1, { toName: 'RSM DAR', imeis: ['351000000000004'], signature: TR_PNG });
  const r = await _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'sender', reason: 'AGENT TWO tapped by mistake, not AGENT ONE' });
  assert.equal(r.fromStatus, 'sent'); assert.equal(r.restored, 0, 'nothing had moved yet');
  const row = d._dump('transfers').find(t => t.id === s.id);
  assert.equal(row.sender_signed_by, null); assert.equal(row.sender_signed_at, null); assert.equal(row.sender_signature, null);
  assert.equal(row.status, 'sent');
  assert.equal(row.reversed_role, 'sender'); assert.equal(row.reversed_by, 'SIPHO K');
  assert.match(row.reversal_reason, /not AGENT ONE/); assert.equal(row.reversal_count, 1);
  assert.equal(rowOf(d, 'devices', '351000000000004').holder, 'AGENT ONE', 'never moved -- correctly untouched');

  // The document is genuinely open again: the sender signs through the SAME door as ever.
  const sg = await _FNS.transferSign(d, AG1, { id: s.id, role: 'sender', signature: TR_PNG });
  assert.equal(sg.signedBy, 'AGENT ONE');
  assert.equal(d._dump('transfers').find(t => t.id === s.id).sender_signed_by, 'AGENT ONE');
});

test('reversing the receiver\'s signature on an ACCEPTED document sends the stock back the way it came', async () => {
  const d = trDb();
  const s = await _FNS.transferCreate(d, STORE, { toName: 'RSM DAR', imeis: ['351000000000001', '861000000000001'] });
  await _FNS.transferAccept(d, RSM_DAR, { id: s.id, signature: TR_PNG });
  assert.equal(rowOf(d, 'devices', '351000000000001').holder, 'RSM DAR');
  assert.equal(rowOf(d, 'old_stock', '861000000000001').agent, 'RSM DAR');
  assert.equal(d._dump('transfers').find(t => t.id === s.id).status, 'accepted');

  const r = await _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'receiver', reason: 'RSM DAR never actually received these -- signed under pressure' });
  assert.equal(r.fromStatus, 'accepted'); assert.equal(r.restored, 2, 'both handsets sent back');
  assert.equal(rowOf(d, 'devices', '351000000000001').holder, 'SUPER AGENT', 'back with the desk, exactly where it came from');
  assert.equal(rowOf(d, 'old_stock', '861000000000001').agent, 'SUPER AGENT');
  assert.equal(rowOf(d, 'old_stock', '861000000000001').rsm, null, 'the desk answers to nobody, same rule a forward move to the desk uses');
  const row = d._dump('transfers').find(t => t.id === s.id);
  assert.equal(row.status, 'sent'); assert.equal(row.receiver_signed_by, null);
  assert.equal(row.accepted_at, null); assert.equal(row.accepted_by, null); assert.equal(row.moved, null);
  const trail = d._dump('device_events').filter(e => e.imei === '351000000000001');
  assert.equal(trail.length, 2, 'the forward move and the reversal each leave their own line');
  assert.match(trail[1].reason, /RSM DAR.*SUPER AGENT/, 'the reversal\'s own trail narrates the direction it actually moved');

  // Corrected: the RIGHT signature lands, and completing it moves the stock again, correctly.
  const acc2 = await _FNS.transferAccept(d, RSM_DAR, { id: s.id, signature: TR_PNG });
  assert.equal(acc2.moved, 2);
  assert.equal(rowOf(d, 'devices', '351000000000001').holder, 'RSM DAR');
  assert.equal(d._dump('transfers').find(t => t.id === s.id).status, 'accepted');
});
...
test('reversing one of two signatures on a COMPLETED three-way document leaves the other in place', async () => {
  const d = trDb();
  const r = await _FNS.transferCreate(d, STORE, { toName: 'RSM MWANZA', fromName: 'RSM DAR', imeis: ['351000000000003'], signature: TR_PNG });
  await _FNS.transferAccept(d, RSM_MWZ, { id: r.id, signature: TR_PNG });
  await _FNS.transferSign(d, RSM_DAR, { id: r.id, role: 'sender', signature: TR_PNG });   // completes it
  assert.equal(rowOf(d, 'devices', '351000000000003').holder, 'RSM MWANZA');
  assert.equal(d._dump('transfers').find(t => t.id === r.id).status, 'accepted');

  // The RECEIVER's acceptance was the wrong person -- reverse only that one.
  const rev = await _FNS.transferReverseSignature(d, STORE, { id: r.id, role: 'receiver', reason: 'a different Mwanza officer accepted by mistake' });
  assert.equal(rev.restored, 1);
  assert.equal(rowOf(d, 'devices', '351000000000003').holder, 'RSM DAR', 'back with the source RSM');
  const row = d._dump('transfers').find(t => t.id === r.id);
  assert.equal(row.status, 'sent');
  assert.equal(row.receiver_signed_by, null, 'the corrected signature is gone');
  assert.equal(row.sender_signed_by, 'RSM DAR', 'the SOURCE RSM\'s approval was never in question -- untouched');
  assert.equal(row.desk_signed_by, 'SIPHO K', 'and neither was the desk\'s own filing signature');

  // The correct destination RSM accepts, and because the source RSM's approval is still on
  // file, THIS is the signature that finishes it again -- exactly transferAccept's own rule.
  const acc2 = await _FNS.transferAccept(d, RSM_MWZ, { id: r.id, signature: TR_PNG });
  assert.equal(acc2.pending, undefined); assert.equal(acc2.moved, 1);
  assert.equal(rowOf(d, 'devices', '351000000000003').holder, 'RSM MWANZA');
  assert.equal(d._dump('transfers').find(t => t.id === r.id).status, 'accepted');
});

test('reversal still corrects the row without the migration, but says the reason cannot show on the document yet', async () => {
  const d = trDb({}, { missingColumns: { transfers: ['reversed_role', 'reversed_by', 'reversed_at', 'reversal_reason', 'reversal_count'] } });
  const s = await _FNS.transferCreate(d, AG1, { toName: 'RSM DAR', imeis: ['351000000000004'], signature: TR_PNG });
  const r = await _FNS.transferReverseSignature(d, STORE, { id: s.id, role: 'sender', reason: 'wrong tap' });
  assert.equal(r.ok, true); assert.equal(r.loggedOnDocument, false);
  assert.match(r.note, /RUN-ME-2026-09-23-transfer-reversal\.sql/);
  const row = d._dump('transfers').find(t => t.id === s.id);
  assert.equal(row.sender_signed_by, null, 'the actual correction still landed');
  assert.equal(row.status, 'sent');
  assert.ok(!('reversed_role' in row), 'the trail columns were correctly never sent');

  // The list still opens on the same database, cascading past the reversal columns exactly
  // as it already does past the three-way and flow columns.
  const list = await _FNS.transferList(d, STORE, {});
  assert.equal(list.notReady, false); assert.equal(list.rows[0].reversalCount, 0, 'absent reads as the safe default');
});
...
test('the register is each party\'s own; the desk sees every document', async () => {
  ...
  await _FNS.transferCreate(d, RSM_DAR, { toName: 'RSM MWANZA', imeis: ['351000000000003'] });
...
/* =========================================================================================
   THE STAFF REGISTER AND THE ACCESS CODES, FILLED FROM STOCK.
     "always pull them auto from new stock whenever they appear and auto insert them in access
      ...
      stock and old stock data"
   ========================================================================================= */
test('opening NEW STOCK or OLD STOCK mints a code and a staff row for every RSM and agent the stock names', async () => {
  const d = trDb({
    hoop_agents: [{ name: 'RSM DAR', role: 'Regional_Manager', branch: 'Dar es salaam', phone: '0700000001' }],
    access_codes: [{ code: 'S1', name: 'SIPHO K', role: 'STORE' }, { code: 'R1', name: 'rsm dar', role: 'RSM' }],
    old_stock: [
      ...
      { imei: '861000000000002', item: 'ITEL A100', agent: 'AGENT THREE', agent_phone: '', rsm: 'RSM MWANZA', rsm_phone: '0700000002', age_days: 40, as_of: '2026-09-10' },
      { imei: '861000000000003', item: 'ITEL A100', agent: 'SUPER AGENT', rsm: '', age_days: 40, as_of: '2026-09-10' },
      { imei: '861000000000004', item: 'ITEL A100', agent: '-', rsm: '12345', age_days: 40, as_of: '2026-09-10' },
      // RSM MWANZA also carries stock as an "agent" -- an RSM is never demoted by that row.
      { imei: '861000000000005', item: 'ITEL A100', agent: 'RSM MWANZA', agent_phone: '0700000002', rsm: 'RSM MWANZA', age_days: 40, as_of: '2026-09-10' },
    ],
  });
  const r = await _FNS.oldStock(d, STORE, {});
  assert.deepEqual({ staff: r.staffSync.staffAdded, codes: r.staffSync.codesAdded, noPhone: r.staffSync.noPhone },
    { staff: 2, codes: 3, noPhone: 1 }, 'AGENT ONE and RSM MWANZA get rows (phones); AGENT THREE a code only (no phone)');
  const codes = d._dump('access_codes');
  const byName = Object.fromEntries(codes.map(c => [c.name, c]));
  assert.equal(byName['AGENT ONE'].role, 'AGENT'); assert.equal(byName['AGENT THREE'].role, 'AGENT');
  assert.equal(byName['RSM MWANZA'].role, 'RSM', 'an RSM on one row and an "agent" on another is an RSM');
  assert.ok(!byName['SUPER AGENT'] && !byName['-'] && !byName['12345'], 'the warehouse and spreadsheet noise are not people');
  assert.equal(codes.filter(c => /rsm dar/i.test(c.name)).length, 1, 'a name that already has a code, however spelled, gets nothing');
  assert.ok(codes.every(c => /^[23456789ABCDEFGHJKMNPQRSTUVWXYZ]{6}$/.test(c.code) || ['S1', 'R1'].includes(c.code)),
    'minted from the phone-safe alphabet, like a team code');
  assert.ok(codes.filter(c => c.name === 'AGENT ONE').every(c => Array.isArray(c.tabs) && !c.tabs.length && c.teams === null),
    'no navs of its own: the role\'s ticks are the grant');
  const staff = d._dump('hoop_agents');
  const ag1 = staff.find(s => s.name === 'AGENT ONE');
  assert.equal(ag1.role, 'Field_Officer'); assert.equal(ag1.manager, 'RSM DAR', '"when we match rsm and agent on the same imei then thats done"');
  assert.equal(ag1.phone, '0700000011'); assert.equal(ag1.active, true);
  assert.equal(staff.find(s => s.name === 'RSM MWANZA').role, 'Regional_Manager');
  assert.ok(!staff.find(s => s.name === 'AGENT THREE'), 'no phone, no register row -- the register is keyed by phone');

  // Idempotent: a second open adds nobody, and a view-only code writes nothing at all.
  const again = await _FNS.oldStock(d, STORE, {});
  assert.deepEqual({ staff: again.staffSync.staffAdded, codes: again.staffSync.codesAdded }, { staff: 0, codes: 0 });
  const VIEW = { code: 'V', name: 'Auditor', role: 'AUDITOR', teams: null, tabs: TR_TABS, readOnly: true };
  const e = trDb({ access_codes: [{ code: 'S1', name: 'SIPHO K', role: 'STORE' }] });
  const before = e._dump('access_codes').length;
  const v = await _FNS.oldStock(e, VIEW, {});
  assert.equal(v.staffSync.codesAdded, 0); assert.equal(e._dump('access_codes').length, before);
  // And NEW STOCK does the same off the register's own rsm/agent columns.
  const n = trDb({ access_codes: [{ code: 'S1', name: 'SIPHO K', role: 'STORE' }], hoop_agents: [] });
...
test('transferUsers offers exactly the people the sync made, so Send can pick by role then name', async () => {
  const d = trDb({ access_codes: [{ code: 'S1', name: 'SIPHO K', role: 'STORE' }] });
  assert.equal((await _FNS.transferUsers(d, STORE, {})).users.length, 0, 'nobody yet');
  await _FNS.oldStock(d, STORE, {});
  const after = await _FNS.transferUsers(d, STORE, {});
  assert.deepEqual(after.users.map(u => u.role + ':' + u.name).sort(), ['AGENT:AGENT ONE', 'AGENT:AGENT THREE', 'RSM:RSM DAR', 'RSM:RSM MWANZA']);
});
...
test('a bulk list opens one signed document per receiver, and nothing until every line passes', async () => {
  ...
  assert.ok(docs.every(t => t.sender_signed_by === 'SIPHO K' && t.status === 'sent' && t.from_name === 'SIPHO K'),
    'each carries the sender\'s signature, is from the login, and waits');
  assert.equal(d._dump('transfer_items').length, 3);
  assert.equal(rowOf(d, 'devices', '351000000000001').holder, 'SUPER AGENT', 'still nothing moves before acceptance');
...
test('a dry run of transferCreate answers every question and writes nothing', async () => {
  ...
  assert.deepEqual(r, { ok: true, dryRun: true, fromName: 'AGENT ONE', toName: 'RSM DAR', count: 1, unknown: 0, priced: 1 });
