...
async function beat(db, [payload], nowMs) {
  ...
  return {
    ...
    /* DROPPED: DEVICE_LOCK_REASON, the fallback line a self-lock used to show.
       "DROP THE REASON FILLING AND ITS DATA SINCE THE MESSAGE IS ENOUGH" -- an ORDERED
       lock still carries its own reason (the portal refuses to send one without) and that
       is untouched. This is only the self-lock case, where the message above already says
       whose phone this is and what to do with it; the REASON line is simply blank now
       rather than restating that in different words. */
